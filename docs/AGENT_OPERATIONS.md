...
- For manual runtime `.env` mutations, prefer `scripts/safe-update-api-env.sh` (backup + single-key update + `.env` guardrail + optional restart/health verify) instead of ad-hoc inline editors.
