...
- MyDevil has a temporary safety cron at `/usr/home/vetternkraft/bin/cleanup-codex-remote-payload.sh` that removes stopped `CODEX_REMOTE_PAYLOAD` shells once per minute. Its repo source is `scripts/mydevil/cleanup-codex-remote-payload.sh`. Before termination it logs the target and parent process metadata (including PGID/SID/TTY/TPGID/wchan/xstat and command) to the owner-only cleanup log; it never logs process environments. Treat it as a guardrail, not the primary fix.
