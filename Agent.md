...
- Push-triggered deploys check the complete pushed commit range with
  `scripts/ci/detect-deploy-changes.sh`; do not reduce checkout depth, because
  multi-commit pushes otherwise look like force-pushes and deploy every app.
- Frontend extraction uses `scripts/mydevil/extract-frontend-bundle.sh`. It
  rolls back failed extraction and retains the immediately previous
  fingerprinted static assets so browser tabs opened during a deploy do not
  lose CSS/JS.
...
- MyDevil has a temporary safety cron at `/usr/home/vetternkraft/bin/cleanup-codex-remote-payload.sh` that removes stopped `CODEX_REMOTE_PAYLOAD` shells once per minute. Its repo source is `scripts/mydevil/cleanup-codex-remote-payload.sh`. Before termination it logs the target and parent process metadata (including PGID/SID/TTY/TPGID/wchan/xstat and command) to the owner-only cleanup log; it never logs process environments. The kill phase is restricted to PIDs from that diagnostic batch and revalidates stopped state plus the Codex marker immediately before both TERM and KILL. Set `CODEX_PAYLOAD_CLEANUP_DRY_RUN=1` to capture the same diagnostics without sending signals. Treat it as a guardrail, not the primary fix.
