...
- MyDevil has a temporary safety cron at `/usr/home/vetternkraft/bin/cleanup-codex-remote-payload.sh` that removes stopped `CODEX_REMOTE_PAYLOAD` shells once per minute. Its repo source is `scripts/mydevil/cleanup-codex-remote-payload.sh`. Treat it as a guardrail, not the primary fix.
