...
const VISUAL_DIFF_ACTION_IDS = new Set(['01-extension-list', '02-extension-detail']);
...
async function loginVersum(page: any) {
    ...
    await page.goto('https://panel.versum.com/salonblackandwhite/extension/', {
        waitUntil: 'domcontentloaded',
    });
...
function buildReport(results: AuditResult[], pixelDiffResults: PixelDiffResult[]) {
    const lines: string[] = [];
    lines.push('# Extension Parity Audit (Production, Full)');
    lines.push('');
    ...
    lines.push('');
    const functionalParity = results.every((x) => x.parity === 'YES')
        ? 'YES'
        : 'NO';
    const visualParity = pixelDiffResults.every((x) => x.pass) ? 'YES' : 'NO';
    lines.push('## Final Verdict');
    lines.push(`- Functional parity: \`${functionalParity}\``);
    ...
    lines.push('- Screenshots: `panel-*.png`, `versum-*.png`, `diff-*.png`');
...
const actions: AuditAction[] = [
    {
        id: '01-extension-list',
        name: 'Extension list',
        panelUrl: 'https://panel.salon-bw.pl/extension',
        versumUrl: 'https://panel.versum.com/salonblackandwhite/extension/',
        panelCheck: {
            requiredTexts: [
                'dodatki',
            ...
            requiredSelectors: ['#sidebar', '#main-content', '.versum-extension-grid', '.ext-col'],
            forbiddenTexts: ['this page could not be found', 'application error'],
        },
        versumCheck: {
            requiredTexts: [
                'dodatki',
            ...
            forbiddenTexts: ['404', 'application error'],
        },
    },
    {
        id: '02-extension-detail',
        name: 'Extension detail (automatic marketing)',
        panelUrl: 'https://panel.salon-bw.pl/extension/tools/4',
        versumUrl:
            'https://panel.versum.com/salonblackandwhite/extension/tools/4',
        panelCheck: {
            requiredTexts: ['marketing automatyczny', 'dostępność'],
            requiredSelectors: ['#sidebar', '#main-content', '.extension_info', '.availability-table'],
            forbiddenTexts: ['this page could not be found', 'application error'],
        },
        versumCheck: {
            requiredTexts: ['marketing automatyczny', 'dostępność'],
...
test.describe('PROD audit: extension panel vs versum', () => {
    ...
    test('generate full checklist and screenshots', async ({ browser }) => {
        ...
        const outDir = path.resolve(
            process.cwd(),
            `../../output/parity/${runDate}-extension-prod-full`,
        );
        const baselineDir = path.resolve(
            process.cwd(),
            `../../output/parity/${runDate}-extension-visual-baseline`,
        );
