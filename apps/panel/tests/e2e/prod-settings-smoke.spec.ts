...
test.describe('PROD smoke: settings navigation', () => {
    ...
    test('settings page loads and exposes secondary nav links', async ({
        page,
    }) => {
        await login(page);
        ...
        await expect(
            nav.getByRole('link', { name: 'Dane salonu' }),
        ).toBeVisible();
        await expect(
            nav.getByRole('link', { name: 'Godziny otwarcia' }),
        ).toBeVisible();
        await expect(nav.getByRole('link', { name: 'Grafiki pracy' })).toBeVisible();
    });
});
