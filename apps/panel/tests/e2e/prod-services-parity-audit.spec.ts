...
function buildReport(results: AuditResult[]) {
    const lines: string[] = [];
    lines.push('# Services Parity Audit (Production, Full)');
    lines.push('');
...
async function resolvePanelServiceId(page: any): Promise<number | null> {
    ...
    if (!href) return null;
    const match = href.match(/\/services\/(\d+)/);
    return match ? Number(match[1]) : null;
}

async function resolveVersumServiceId(page: any): Promise<number | null> {
    await page.goto('https://panel.versum.com/salonblackandwhite/services', { waitUntil: 'domcontentloaded', timeout: 35000 });
    await page.waitForTimeout(1000);
    const link = page.locator('table.versum-table tbody tr td.link_body a[href^="/services/"]:not([href*="tab_name"]):not([href*="edit"])').first();
    const count = await link.count().catch(() => 0);
    if (count === 0) return null;
    const href = await link.getAttribute('href');
    if (!href) return null;
...
function buildActions(panelId: number | null, versumId: number | null): AuditAction[] {
    const list: AuditAction[] = [
        {
            id: '01-services-list',
            name: 'Services list',
            ...
            panelCheck: {
                ...
                requiredSelectors: ['#sidebar', '#main-content', 'table'],
            },
            versumCheck: {
        ...
        },
        {
            ...
            panelCheck: {
                ...
                requiredSelectors: ['#main-content', 'form'],
            },
            versumCheck: {
                requiredTexts: ['nowa usługa'],
                requiredSelectors: ['#main-content', 'form'],
...
test.describe('PROD audit: services panel vs versum', () => {
    ...
    test('generate full checklist and screenshots', async ({ browser }) => {
        ...
        const outDir = path.resolve(
            process.cwd(),
            `../../output/parity/${runDate}-services-prod-full`,
        );
        const baselineDir = path.resolve(
            process.cwd(),
            `../../output/parity/${runDate}-services-visual-baseline`,
        );
        ...
        const panelId = await resolvePanelServiceId(panelPage);
        const versumId = await resolveVersumServiceId(versumPage);

        const results: AuditResult[] = [];
        const pixelDiffResults: PixelDiffResult[] = [];

        for (const [index, action] of buildActions(panelId, versumId).entries()) {
