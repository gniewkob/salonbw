...
const templatePath = path.resolve(repoRoot, 'static_preview/templates/calendar_template.html');
...
function extractCssUrls(cssText, baseUrl) {
  ...
  while ((m = re.exec(cssText)) !== null) {
    ...
    if (!raw || raw.startsWith('data:') || raw.startsWith('#') || raw.startsWith('svg(')) continue;
    if (raw.startsWith('http://') || raw.startsWith('https://') || raw.startsWith('//')) {
      if (raw.includes('app-cdn.versum.net') || raw.startsWith('//app-cdn.versum.net')) {
        urls.add(toAbsoluteUrl(raw));
      }
      continue;
    }
...
async function main() {
  ...
  const initialUrls = new Set([...extractAssetUrls(template), ...extraAssetUrls.map(toAbsoluteUrl)]);
  ...
  const manifest = {
    ...
    assets: [...downloaded.values()].sort((a, b) => a.file.localeCompare(b.file)),
    missing,
  };
  ...
  html = html.replaceAll('/salonblackandwhite/settings/timetable/schedules/', '/settings/timetable/schedules/');
  html = html.replaceAll('/salonblackandwhite/track_new_events.json', '/track_new_events.json');
  html = html.replaceAll('/salonblackandwhite/graphql', '/graphql');
  html = html.replaceAll('https://panel.versum.com/salonblackandwhite', '/dashboard');
