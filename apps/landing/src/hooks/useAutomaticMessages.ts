...
export const AUTOMATIC_MESSAGES_QUERY_KEY = ['api', '/automatic-messages'] as const;
...
export function useAutomaticMessages() {
    ...
    const createMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: AUTOMATIC_MESSAGES_QUERY_KEY });
        },
    });

    // Update rule
    const updateMutation = useMutation({
        mutationFn: async ({ id, data }: { id: number; data: UpdateAutomaticMessageRuleRequest }) => {
    ...
    const toggleMutation = useMutation({
        mutationFn: async (id: number) => {
            return apiFetch<AutomaticMessageRule>(`/automatic-messages/${id}/toggle`, {
                method: 'PATCH',
            });
        },
        onSuccess: () => {
    ...
    const processAllMutation = useMutation({
        mutationFn: async () => {
            return apiFetch<ProcessAutomaticMessagesResult[]>('/automatic-messages/process', {
                method: 'POST',
            });
        },
        onSuccess: () => {
    ...
    const processOneMutation = useMutation({
        mutationFn: async (id: number) => {
            return apiFetch<ProcessAutomaticMessagesResult>(`/automatic-messages/${id}/process`, {
                method: 'POST',
            });
        },
        onSuccess: () => {
