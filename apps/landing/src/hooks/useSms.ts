...
export function useMessageTemplates(options?: {
    ...
    const queryKey = [...TEMPLATES_QUERY_KEY, { type, channel, isActive }] as const;
    ...
    const query = useQuery({
        ...
        queryFn: async () => {
            ...
            return apiFetch<MessageTemplate[]>(`/sms/templates${qs ? `?${qs}` : ''}`);
        },
        enabled,
    });
...
export function useSmsHistory(filter: SmsHistoryFilter) {
    ...
    const query = useQuery({
        ...
        queryFn: async () => {
            ...
            if (filter.appointmentId) params.set('appointmentId', String(filter.appointmentId));
...
export function useSmsMutations() {
    ...
    const updateTemplate = useMutation({
        mutationFn: async ({ id, ...payload }: UpdateTemplatePayload & { id: number }) => {
            return apiFetch<MessageTemplate>(`/sms/templates/${id}`, {
                method: 'PUT',
                body: JSON.stringify(payload),
            });
