...
export function useDeliveries(filters?: DeliveryFilters) {
    ...
    return useQuery<Delivery[]>({
        ...
        queryFn: () => apiFetch<Delivery[]>(`/deliveries${queryStr ? `?${queryStr}` : ''}`),
    });
}
...
export function useStocktakings(filters?: StocktakingFilters) {
    ...
    return useQuery<Stocktaking[]>({
        ...
        queryFn: () => apiFetch<Stocktaking[]>(`/stocktaking${queryStr ? `?${queryStr}` : ''}`),
    });
}
...
export function useStartStocktaking() {
    ...
    return useMutation({
        mutationFn: (id: number) =>
            apiFetch<Stocktaking>(`/stocktaking/${id}/start`, { method: 'POST' }),
        onSuccess: (_, id) => {
            queryClient.invalidateQueries({ queryKey: ['stocktakings'] });
...
export function useAddStocktakingItems() {
    ...
    return useMutation({
        mutationFn: ({
            ...
            items: Array<{ productId: number; countedQuantity: number; notes?: string }>;
        }) =>
        ...
        onSuccess: (_, { stocktakingId }) => {
            queryClient.invalidateQueries({ queryKey: ['stocktaking', stocktakingId] });
        },
    });
}
