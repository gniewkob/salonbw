...
export function useStockAlerts(options: UseStockAlertsOptions = {}) {
    ...
    const { productType, trackStockOnly = true, limit, enabled = true } = options;

    const params = new URLSearchParams();
    if (productType) params.append('productType', productType);
    if (trackStockOnly !== undefined) params.append('trackStockOnly', String(trackStockOnly));
...
export function useSupplierReorderSuggestions(supplierId: number | null, enabled = true) {
    const { apiFetch } = useAuth();

    return useQuery({
        queryKey: [...STOCK_ALERTS_QUERY_KEY, 'suppliers', supplierId, 'reorder'],
        queryFn: async () => {
            return apiFetch<ReorderSuggestion[]>(`/stock-alerts/suppliers/${supplierId}/reorder`);
        },
