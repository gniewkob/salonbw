...
export function useGiftCards(params?: GiftCardQueryParams) {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            if (params?.recipientId) searchParams.append('recipientId', String(params.recipientId));
            if (params?.purchasedById) searchParams.append('purchasedById', String(params.purchasedById));
            if (params?.code) searchParams.append('code', params.code);
            if (params?.page) searchParams.append('page', String(params.page));
            if (params?.limit) searchParams.append('limit', String(params.limit));
            const query = searchParams.toString();
            return apiFetch<{ data: GiftCard[]; total: number }>(`/gift-cards${query ? `?${query}` : ''}`);
        },
    });
}
...
export function useGiftCardTransactions(giftCardId: number | null) {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            return apiFetch<GiftCardTransaction[]>(`/gift-cards/${giftCardId}/transactions`);
        },
        enabled: giftCardId !== null,
    });
}

export function useValidateGiftCard(code: string | null, amount?: number, serviceId?: number) {
    const { apiFetch } = useAuth();

    return useQuery({
        queryKey: [...GIFT_CARDS_QUERY_KEY, 'validate', code, amount, serviceId],
        queryFn: async () => {
            const searchParams = new URLSearchParams();
            if (amount) searchParams.append('amount', String(amount));
            if (serviceId) searchParams.append('serviceId', String(serviceId));
            const query = searchParams.toString();
            return apiFetch<GiftCardValidation>(`/gift-cards/validate/${code}${query ? `?${query}` : ''}`);
        },
        enabled: code !== null && code.length > 0,
    });
}
...
export function useUpdateGiftCard() {
    ...
    return useMutation({
        mutationFn: async ({ id, data }: { id: number; data: UpdateGiftCardRequest }) => {
            return apiFetch<GiftCard>(`/gift-cards/${id}`, {
                method: 'PUT',
                body: JSON.stringify(data),
            });
...
export function useAdjustGiftCardBalance() {
    ...
    return useMutation({
        mutationFn: async ({ id, data }: { id: number; data: AdjustGiftCardBalanceRequest }) => {
