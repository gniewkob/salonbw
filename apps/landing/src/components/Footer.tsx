...
type FooterRouteLinkProps = Omit<
...
function FooterRouteLink({ href, ...props }: FooterRouteLinkProps) {
    const router = useRouter();

    const navigateFromFooter = (event: MouseEvent<HTMLAnchorElement>) => {
        if (
            href.includes('#') ||
            event.button !== 0 ||
            event.metaKey ||
        ...
        void router
            .push(href as Route, undefined, { scroll: false })
            .then(() => {
                window.scrollTo(0, 0);
            })
            .finally(() => {
    ...
    return (
        <Link {...props} href={href as Route} onClick={navigateFromFooter} />
    );
}
        ...
        >
            <div
                style={{
                    height: '1px',
                    background:
                        'linear-gradient(90deg, transparent, var(--brand-silver), transparent)',
                }}
            />
