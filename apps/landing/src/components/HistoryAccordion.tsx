                        ...
                        return (
                            <div key={item.id} style={{ borderTop: idx === 0 ? '1px solid rgba(255,255,255,0.1)' : 'none', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                                <button
                                    ...
                                    className="w-full py-6 flex justify-between items-center text-left transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-[#b8bcc8] focus:ring-inset"
                                    aria-expanded={isOpen}
                                    aria-controls={`history-content-${item.id}`}
                                >
                                    <div className="flex items-center gap-5">
                                        <span className="text-xs font-mono w-10 shrink-0" style={{ color: '#b8bcc8' }}>
                                            {T.history.yearMap[item.id as keyof typeof T.history.yearMap] ?? '—'}
                                        </span>
                                        <h3 className="text-lg font-semibold" style={{ fontFamily: "var(--font-playfair), serif", color: isOpen ? '#b8bcc8' : '#ffffff', transition: 'color 0.2s' }}>
                                            {item.title}
                                        </h3>
                                    </div>
                                    <svg
                                        className="shrink-0 ml-4 transition-transform duration-300"
                                        style={{ transform: isOpen ? 'rotate(180deg)' : 'rotate(0)', color: '#b8bcc8' }}
