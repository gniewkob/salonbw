...
async function initializeCalendar() {
    ...
    if (!el) {
        console.error(
            'Element kalendarza nie został znaleziony! Sprawdź czy jesteś na właściwej stronie.',
        );
        return;
    }
    ...
    if (!url) {
        console.error(
            'Brak URL dla wydarzeń! Sprawdź atrybut data-events-url.',
        );
        return;
    }
    ...
    try {
        const calendar = new Calendar(el, {
            ...
            dateClick(info) {
                ...
                if (!isWithinWorkingHours(info.date)) {
                    showNotification(
                        'Nie można dodać rezerwacji poza godzinami pracy salonu.',
                    );
                    return;
                }
            ...
            eventClick(info) {
                ...
                if (
                    ...
                    info.event.extendedProps.is_blocker
                ) {
                    return;
                }
                if (window.modalIsOpen) {
                ...
                window.dispatchEvent(
                    new CustomEvent('open-view-modal', { detail: data }),
                );
            },
            editable: true,
            eventDrop: function (info) {
                ...
                if (!isWithinWorkingHours(info.event.start)) {
                    ...
                    showNotification(
                        'Nie można przenieść rezerwacji poza godziny pracy salonu.',
                    );
                    return;
                }
                ...
                if (
                    !confirm('Czy na pewno chcesz zmienić termin rezerwacji?')
                ) {
                    info.revert();
                    return;
                }
                ...
                fetch(
                    `/admin/kalendarz/appointments/${appointmentId}/update-time`,
                    {
                        method: 'POST',
                        headers: {
                    ...
                    .then((data) => {
                        if (data.success) {
                            ...
                            if (data.extendedProps) {
                                Object.entries(data.extendedProps).forEach(
                                    ([key, val]) => {
                            ...
                            showNotification(
                                'Termin rezerwacji został zaktualizowany.',
                            );
                        } else {
                            info.revert();
                            showNotification(
                                'Wystąpił błąd podczas aktualizacji terminu.',
                            );
                        }
                    })
                    .catch((error) => {
            ...
            eventResize: function (info) {
                ...
                if (!isWithinWorkingHours(info.event.end)) {
                    info.revert();
                    showNotification(
                        'Rezerwacja nie może wykraczać poza godziny pracy salonu.',
                    );
                    return;
                }
                ...
                showNotification(
                    'Zmiana długości rezerwacji nie jest obecnie obsługiwana.',
                );
            },
        ...
        infoElement.innerHTML =
            '<i class="fas fa-info-circle"></i> Możesz przeciągać wydarzenia, aby zmienić termin rezerwacji.';
        ...
        showNotification(
            'Wystąpił błąd podczas inicjalizacji kalendarza. Odśwież stronę.',
        );
        return null;
    }
...
setTimeout(function () {
    ...
    if (el && !el._fullCalendar) {
        ...
        button.className =
            'px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 mb-4';
