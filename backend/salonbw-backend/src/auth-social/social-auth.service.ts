...
export class SocialAuthService {
    ...
    async handleSocialLogin(
        ...
        if (!profile?.email) {
            throw new UnauthorizedException('No email provided from social provider');
        }
        ...
        if (!user) {
            ...
            if (existingUser) {
                ...
                await this.linkSocialAccount(existingUser.id, provider, profile.id);
    ...
    async getLinkedSocialAccounts(userId: number): Promise<{ google: boolean; facebook: boolean }> {
    ...
    private async findUserBySocialId(provider: 'google' | 'facebook', socialId: string): Promise<User | null> {
    ...
    private async linkSocialAccount(userId: number, provider: 'google' | 'facebook', socialId: string): Promise<void> {
    ...
    private async createSocialUser(data: {
        ...
        const userData = {
            ...
            ...(data.provider === 'google' ? { googleId: data.socialId } : { facebookId: data.socialId }),
        };
