...
- **Multi-role panel UX/UI review DONE 2026-06-16** (live on prod, all 3 roles via test.admin@/test.ola@/test.klient@). Findings + fixes:
  ...
  - Layout/brand notes (not yet fixed, low pri): admin left filter sidebars (customers GRUPY KLIENTÓW / WYBIERZ KRYTERIA, services categories) render as bare black-outlined boxes vs brand silver pills; client /booking shows staff shell sidebar (mini-cal + empty PRACOWNICY) clutter; some services show description==name duplicated. Booking content itself = clean/on-brand.
