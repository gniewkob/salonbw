                    ...
                    pinoHttp: {
                        ...
                        genReqId: (req, res) => {
                            ...
                            const value = Array.isArray(header)
                                ? header[0]
                                : header;
                            const requestId = value && typeof value === 'string' ? value : (0, uuid_1.v4)();
...
function createThrottlerConfig(config) {
    const ttl = asPositiveNumber(config.get('THROTTLE_TTL', '60000'), 'THROTTLE_TTL');
    const limit = asPositiveNumber(config.get('THROTTLE_LIMIT', '10'), 'THROTTLE_LIMIT');
    return [{ ttl, limit }];
}
function createTypeOrmConfig(config) {
    const dbUrl = config.get('DATABASE_URL');
    const shouldSync = config.get('DB_SYNCHRONIZE', 'false').toLowerCase() === 'true';
    const nodeEnv = config.get('NODE_ENV', 'development');
    if (nodeEnv === 'production' && shouldSync) {
        throw new Error('DB_SYNCHRONIZE must be false in production for safety');
    }
    if (!dbUrl) {
    ...
    const connectionTimeoutMillis = config.get('DB_CONNECTION_TIMEOUT_MS', 5000);
